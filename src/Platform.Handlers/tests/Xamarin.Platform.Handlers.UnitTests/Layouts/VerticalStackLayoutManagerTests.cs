...
namespace Xamarin.Platform.Handlers.UnitTests.Layouts
{
	...
	public class VerticalStackLayoutManagerTests : StackLayoutManagerTests
	{
		...
		public void SpacingMeasurement(int viewCount, float viewHeight, int spacing, float expectedHeight)
		{
			...
			var measuredSize = manager.Measure(100, float.PositiveInfinity);
		...
		public void SpacingArrangementOneItem(int spacing)
		{
			...
			var measuredSize = manager.Measure(100, float.PositiveInfinity);
			manager.Arrange(new RectangleF(PointF.Zero, measuredSize));

			var expectedRectangle = new RectangleF(0, 0, 100, 100);
		...
		public void SpacingArrangementTwoItems(int spacing)
		{
			...
			var measuredSize = manager.Measure(float.PositiveInfinity, 100);
			manager.Arrange(new RectangleF(PointF.Zero, measuredSize));

			var expectedRectangle0 = new RectangleF(0, 0, 100, 100);
			stack.Children[0].Received().Arrange(Arg.Is(expectedRectangle0));

			var expectedRectangle1 = new RectangleF(0, 100 + spacing, 100, 100);
		...
		public void StackAppliesHeight(float viewHeight, float stackHeight, float expectedHeight) 
		{
			var stack = CreateTestLayout();

			var view = CreateTestView(new SizeF(100, viewHeight));
			...
			var manager = new VerticalStackLayoutManager(stack);
			var measurement = manager.Measure(100, float.PositiveInfinity);
		...
		public void ViewsArrangedWithDesiredWidths()
		{
			...
			var view1 = CreateTestView(new SizeF(200, 100));
			var view2 = CreateTestView(new SizeF(150, 100));

			var children = new List<IView>() { view1, view2 }.AsReadOnly();
			stack.Children.Returns(children);

			var measurement = manager.Measure(float.PositiveInfinity, float.PositiveInfinity);
			manager.Arrange(new RectangleF(PointF.Zero, measurement));
			...
			var expectedRectangle1 = new RectangleF(0, 0, 200, 100);
			...
			var expectedRectangle2 = new RectangleF(0, 100, 150, 100);
